...
export function createChatNode(llmClient: OpenRouterService, preferencesService: PreferencesService) {
  return async (state: GraphState, runtime?: Runtime): Promise<Partial<GraphState>> => {
    ...
    const result = await llmClient.generateStructured(systemPrompt,
      userPrompt, ChatResponseSchema,)

    if (!result.success || !result.data) {
      console.error(`❌ Falha ao gerar respostas: ${result.error}`)
      return {
        messages: [
    ...
    const totalMesssages = state.messages.length;

    const needsSummarization = totalMesssages >= config.maxMessagesToSummary;
